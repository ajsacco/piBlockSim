...
void setup(){
  String input = javax.swing.JOptionPane.showInputDialog("How many digits of Pi?"); //<>//
...
int intPane(String query){
  try {
    return Integer.parseInt(javax.swing.JOptionPane.showInputDialog(query));
  } catch (Exception e){
    return -1;
  }
}
...
float fPane(String query){
  try {
    return Float.parseFloat(javax.swing.JOptionPane.showInputDialog(query));
  } catch (Exception e){
    return -1.0;
  }
}
...
double dPane(String query){
  try {
    return Double.parseDouble(javax.swing.JOptionPane.showInputDialog(query));
  } catch (Exception e){
    return -1.0;
  }
}
...
void end(){
  javax.swing.JOptionPane.showMessageDialog(null, "pi = " + piFormat(num));
}
